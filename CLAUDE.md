...
| Skills | `skills/*/SKILL.md` | Directory with `SKILL.md` (YAML frontmatter) + optional files | `translate-en` symlinks into the `translate-tool` git submodule (run `git submodule update --init` after cloning); `git-ai-search`/`prompt-analysis` symlink into `~/.git-ai/skills` (disposition is an open decision) |
...
- **Environment variable handling** -- `envsubst.py` expands `${VAR}` from `os.environ`. Claude passes MCP `env` and `headers` values through VERBATIM (deliberate, MCP launcher-bridge design: `${VAR}` expands at runtime via the wrapper's `--mcp-config`, so secrets never land in deployed `settings.json`; do NOT reintroduce deploy-time expansion -- see `docs/superpowers/specs/2026-06-12-mcp-launcher-bridge-design.md`). Droid copies MCP `env` and `headers` verbatim and expands only the models provider `api_key` via lenient `expand_env_vars`. OpenCode expands at deploy time via `expand_env_vars_strict` for `models.providers.*.api_key`, `mcp.*.env.*`, and `mcp.*.headers.*` -- a missing var raises `EnvVarError` and the CLI exits 1, since OpenCode runs from a directory that won't have those vars set.
...
The flake also exports `homeManagerModules.default` (`nix/hm-module.nix`): with `programs.promptdeploy.enable` set, it runs `promptdeploy deploy --quiet` from `sourceDir` on every home-manager activation. Caveat: the activation script swallows failures (`|| true`), so a broken deploy passes silently (pending a maintainer decision).

## CI

`.github/workflows/ci.yml` runs `nix flake check` which executes all 5 checks defined in `flake.nix`: `ruff format --check`, `ruff check`, `mypy`, `pytest` with 100% coverage gate, and `nix build`. `lefthook.yml` mirrors those as pre-commit checks with fast staged-file feedback, plus `nix flake check` as the authoritative full-tree gate, plus an `agnix` lint pass over Markdown/YAML/JSON/TOML files -- a sixth gate that is not part of the flake checks.
