...
- **Environment variable handling** -- `envsubst.py` expands `${VAR}` from `os.environ`. Claude passes MCP `env` and `headers` values through VERBATIM (deliberate, MCP launcher-bridge design: `${VAR}` expands at runtime via the wrapper's `--mcp-config`, so secrets never land in deployed `settings.json`; do NOT reintroduce deploy-time expansion -- see `docs/superpowers/specs/2026-06-12-mcp-launcher-bridge-design.md`). Droid copies MCP `env` verbatim and expands only the models provider `api_key` via lenient `expand_env_vars`. OpenCode expands at deploy time via `expand_env_vars_strict` for both `models.providers.*.api_key` and `mcp.*.env.*` -- a missing var raises `EnvVarError` and the CLI exits 1, since OpenCode runs from a directory that won't have those vars set.
