...
- **Environment variable handling** -- `envsubst.py` expands `${VAR}` from `os.environ`. Claude expands MCP `env` values at deploy time via the lenient `expand_env_in_dict` (deliberate, commit 5731599: Claude Code does not interpolate MCP env itself, and the expanded values land in the `settings.json` rsynced to remote hosts); unset vars are left as literal `${VAR}`. Droid copies MCP `env` verbatim and expands only the models provider `api_key` via lenient `expand_env_vars`. OpenCode expands at deploy time via `expand_env_vars_strict` for both `models.providers.*.api_key` and `mcp.*.env.*` -- a missing var raises `EnvVarError` and the CLI exits 1, since OpenCode runs from a directory that won't have those vars set.
