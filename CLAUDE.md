...
| MCP Servers | `mcp/*.yaml` | YAML with `name`, transport (`command`+`args` or `url`+`headers`), `env`, `scope` | Schema in `mcp/schema.md`; on claude targets effective only via the MCP launcher bridge (see Key Patterns) |
...
- **MCP launcher bridge (claude targets)** -- Claude Code itself never reads the `mcpServers` key promptdeploy writes into `settings.json` (its MCP read surfaces are `.claude.json`, project `.mcp.json`, plugins, claude.ai connectors, and `--mcp-config`). The deployed key takes effect because the `ai` wrapper (`~/src/scripts/ai`, OUTSIDE this repo, load-bearing) appends `--mcp-config "$CLAUDE_CONFIG_DIR/settings.json"` to every claude launch when that file exists (no `--strict-mcp-config`, so plugins and project `.mcp.json` still load); a launch that bypasses the wrapper gets no promptdeploy-managed servers. Standard per-profile verification probe: `claude --strict-mcp-config --mcp-config=<profile>/settings.json -p "Say ok" --output-format json --max-turns 1`. Spec: `docs/superpowers/specs/2026-06-12-mcp-launcher-bridge-design.md`.
- **Environment variable handling (`${VAR}` policy per target)** -- `envsubst.py` expands `${VAR}` from `os.environ` (after `.env` auto-load). Claude: MCP `env` and `headers` pass through VERBATIM -- `${VAR}` expands at runtime on the wrapper's `--mcp-config` surface, so secrets never land in deployed `settings.json`; do NOT reintroduce deploy-time expansion (see the launcher-bridge bullet above). Caveat: an unset variable expands to EMPTY at runtime there, hence the validate warning below. Droid: MCP `env`/`headers` verbatim; only the models provider `api_key` expands, via lenient `expand_env_vars` (unset vars stay as literal `${VAR}` with a stderr warning). OpenCode: strict deploy-time expansion via `expand_env_vars_strict` for `models.providers.*.api_key`, `mcp.*.env.*`, and `mcp.*.headers.*` -- a missing var raises `EnvVarError` and the CLI exits 1, since OpenCode runs from a directory that won't have those vars set. `promptdeploy validate` warns when an MCP `env`/`headers` value references a `${VAR}` not declared in `.env.example` (no `.env.example` disables the check).
