...
- **No-secret-on-error (structural)**: the merge program's body is wrapped in an outer `try/except BaseException -> _fail(...)`; `_fail` emits only fixed strings. `ssh_stdin` interpolates only remote `stderr` (which carries no payload) + host, and NEVER the `script` text. The home-manager `deploy.log` therefore never captures a secret. Locked by tests 2/53b.
